...
# MultiwayToLTS: Bridge between MultiwaySystem and FinLTS

This file bridges the two core formalisms in the project:
- **MultiwaySystem** (Basic.lean): objects of RuleSys, with typed step relation `Step : State → State → Type v`
- **FinLTS** (PresheafTopos/LabeledLTS.lean): finite labeled transition systems with `edge : L → Vertex → Vertex → Prop`

The bridge embeds finite MultiwaySystem instances into `FinLTS Unit` (single-label LTS)
by collapsing the typed step relation to a propositional edge predicate via `Nonempty`.

## Main Definitions

- `MultiwaySystem.toLTS`: Convert a finite MultiwaySystem to `FinLTS Unit`
...
- Restricted to `MultiwaySystem.{0, 0}` since `FinLTS` requires `Type` (not `Type u`)
- Requires `Fintype`, `DecidableEq` on State and `Decidable (Nonempty (M.Step s t))`
- The single label `Unit` reflects that MultiwaySystem is unlabeled
...
/-!
## Part 1: Object Map — MultiwaySystem to FinLTS Unit

A `MultiwaySystem.{0, 0}` with finite decidable state and decidable step existence
...
/-- Convert a finite MultiwaySystem with decidable step existence to a single-label FinLTS.
...
def MultiwaySystem.toLTS (M : MultiwaySystem.{0, 0})
...
/-- Lift a simulation between MultiwaySystem instances to an LTS homomorphism between
    their FinLTS images.
...
def Simulation.toLTSHom {M N : MultiwaySystem.{0, 0}}
    [Fintype M.State] [DecidableEq M.State]
...
theorem Simulation.toLTSHom_comp {M N P : MultiwaySystem.{0, 0}}
    [Fintype M.State] [DecidableEq M.State]
    [Fintype N.State] [DecidableEq N.State]
...
theorem Simulation.toLTSHom_init {M N : MultiwaySystem.{0, 0}}
    [Fintype M.State] [DecidableEq M.State]
    [Fintype N.State] [DecidableEq N.State]
    [∀ s t, Decidable (Nonempty (M.Step s t))]
    [∀ s t, Decidable (Nonempty (N.Step s t))]
